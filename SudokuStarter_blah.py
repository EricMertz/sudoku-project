...
def solve(initial_board, forward_checking = False, MRV = False, Degree = False,
    ...
    if forward_checking:
        if MRV:
            return backtrack_FC_MRV(initial_board, legals)
        if LCV:
            return backtrack_FC_LCV(initial_board, legals)
        if Degree:
            return backtrack_FC_Degree(initial_board, legals)
        return backtrack_FC(initial_board, legals)
    if MRV:
        return backtrack_MRV(initial_board, legals)
    if LCV:
        return backtrack_LCV(initial_board, legals)
    if Degree:
        return backtrack_Degree(initial_board, legals)
...
def backtrack_MRV(board, legals):
    global counter
    if is_complete(board):
        #print counter
        return True, board
    ...
    for val in ls:
        if legals[row][col][val] == 1:
            ...
            new_board = new_board.set_value(row, col, val)
            newLegals, sent = make_3D_move(deepcopy(legals), row, col, val, size)
            worked, board = backtrack_MRV(new_board, newLegals)
            if worked:
                return True, board
    return False, board

# TODO find degree function
def backtrack_Degree(board, legals):
    global counter
    if is_complete(board):
        #print counter
        return True, board
    ...
    for val in ls:
        if legals[row][col][val] == 1:
            ...
            new_board = new_board.set_value(row, col, val)
            newLegals, sent = make_3D_move(deepcopy(legals), row, col, val, size)
            worked, board = backtrack_Degree(new_board, newLegals)
            if worked:
                return True, board
    return False, board

def backtrack_LCV(board, legals):
    global counter
    if is_complete(board):
        #print counter
        return True, board
...
def backtrack_FC(board, legals):
    ...
    for val in ls:
        if consistent_move(legals, row, col, val, size):
            ...
            new_board = new_board.set_value(row, col, val)
            newLegals, sent = make_3D_move(deepcopy(legals), row, col, val, size)
            worked, board = backtrack_FC(new_board, newLegals)
            if worked:
                return True, board
    return False, board


def backtrack_FC_MRV(board, legals):
    global counter
    if is_complete(board):
        #print counter
        return True, board
    ...
    for val in ls:
        if consistent_move(legals, row, col, val, size):
            ...
            new_board = new_board.set_value(row, col, val)
            newLegals, sent = make_3D_move(deepcopy(legals), row, col, val, size)
            worked, board = backtrack_FC_MRV(new_board, newLegals)
            if worked:
                return True, board
    return False, board


def backtrack_FC_LCV(board, legals):
    global counter
    if is_complete(board):
        #print counter
        return True, board
    ...
    for val in ls:
        if consistent_move(legals, row, col, val, size):
            ...
            new_board = new_board.set_value(row, col, val)
            newLegals, sent = make_3D_move(deepcopy(legals), row, col, val, size)
            worked, board = backtrack_FC_LCV(new_board, newLegals)
            if worked:
                return True, board
    return False, board

# find degree
def backtrack_FC_Degree(board, legals):
    global counter
    if is_complete(board):
        #print counter
        return True, board
    ...
    for val in ls:
        if consistent_move(legals, row, col, val, size):
            ...
            new_board = new_board.set_value(row, col, val)
            newLegals, sent = make_3D_move(deepcopy(legals), row, col, val, size)
            worked, board = backtrack_FC_Degree(new_board, newLegals)
            if worked:
                return True, board
    return False, board
